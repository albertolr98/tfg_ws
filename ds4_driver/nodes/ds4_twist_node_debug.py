...
class StatusToTwist(object):
    def __init__(self, node):
        ...
        for attr in Status.__slots__:
            ...
            if attr.startswith("_axis_") or attr.startswith("_button_") or attr.startswith("_imu"):
        ...
        for vel_type in self._inputs.keys():
            ...
            for k, expr in self._inputs[vel_type].items():              
                ...
                try:
                    ...
                    if expr == "imu.linear_acceleration.z":
                        val_z = val
                    elif expr == "imu.linear_acceleration.x":
                        val_x = val
            ...
            for k, expr in self._inputs[vel_type].items():
                val = eval(expr, {}, input_vals)     
                if expr == "imu.linear_acceleration.x":
                    if scale is None:
                        scale = 1.0
                    try:
                        val = eval(expr, {}, input_vals)
